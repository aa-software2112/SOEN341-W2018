...
router.post("/favorite", function(req, res) {
	...
	db.query(sqlFavoriteExist, [req.body.q_id], function(err, result_qfav) {
			if (err) {
...
router.post("/answer-vote", function(req, res) 
{
	...
	db.query(sqlTotalScore, [answer_id], function(err, result_q1)
	{
		if (err) {
		...
		db.query(sqlScoreExist, [user_id, answer_id], function(err, result_q2)	{
			if (err) {
			...
			if (result_q2.length > 0 ) {
				...
				// Add score to database, table score_answer
				db.query(sqlInsertNewScore, [score, datetime_scored_answer, user_id], function(err,result) {
					if(err) {
...
router.post("/question-vote", function(req, res) {
	...
	db.query(sqlTotalScore, [question_id], function(err, result_q1) {
		if (err) {
			console.log("Select Sum query failed " + err);
			return;
		}
		...
		db.query(sqlScoreExist, [user_id, question_id], function(err, result_q2) {
			if (err) {
				console.log("select score query failed " + err);
				return;
			}
			...
			if (result_q2.length > 0 ) {
				...
				if (result_q2.score == 1 && score ==1 || result_q2.score == -1 && score == -1) {
					...
					db.query(sqlInsertNewScore, [score, datetime_scored_question, user_id], function(err,result) {
						if(err) {
							console.log("update score query failed " + err);
							return;
						}

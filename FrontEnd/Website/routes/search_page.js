...
router.get('/search_on', (req,res) => {
	...
	*/
	var search_keyword = 'SELECT question.question_id, question.question_title, question.user_id, user.user_id, user.username, question.datetime_asked FROM question, user WHERE Match(question_title) Against("'+ keyword +'" IN NATURAL LANGUAGE MODE) AND question.user_id = user.user_id';
	var output = ' ';

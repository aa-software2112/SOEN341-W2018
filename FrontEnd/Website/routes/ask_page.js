...
router.post("/askform/:user_id" , (req,res) => {
	...
	var sql_insertQuestion =" insert into question set ?";

	db.query(sql_insertQuestion, newQ, function(err,result){
		if(err){
			console.log(err);
			return;
		}
    ...
    var sql_selectQuestion ="select question.question_id from question JOIN user ON question.user_id = user.user_id \
    WHERE user.user_id = ?";
	...
	db.query(sql_selectQuestion,[req.params.user_id], function(err,qId){

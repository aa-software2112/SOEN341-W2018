...
Remember that some functionalities will not display correctly locally, because Our database entries are different. When you want to try the functionalities. I suggest:

signing up 3-4 new users.
asking at least 10 question (So they are displayed on homepage)
Everytime you ask a question, just make 3-4 answers (the user_id who answers is static for now, so if you want more user names to show, before answer, make sure to change the user id on line 216~!!, you would have to restart the app.js if you are not using nodemon)
